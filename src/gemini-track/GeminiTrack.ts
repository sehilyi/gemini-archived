...
function GeminiTrack(HGC: any, ...args: any[]): any {
  ...
  class GeminiTrackClass extends HGC.tracks.BarTrack {
    ...
    renderTile(tile: any) {
      ...
      if (
        isNotMaxZoomLevel &&
        this.geminiModel.spec().zoomAction?.type === "hide"
      ) {
    ...
    syncMaxAndMin() {
      ...
      visibleAndFetched.map((tile: any) => {
        if (
          tile.minValue + tile.maxValue >
          this.maxAndMin.min + this.maxAndMin.max
        ) {
    ...
    rescaleTiles() {
      ...
      visibleAndFetched.map((tile: any) => {
        ...
        const newZero =
          this.dimensions[1] - valueToPixels(Math.abs(this.maxAndMin.min));
    ...
    localColorToHexScale() {
      const colorScale = [
        ...this.geminiModel.getColorRange(),
        ...this.geminiModel.getColorRange(true),
      ];
    ...
    tabularizeTile(tile: any) {
      ...
      const isStackedBarChart = this.geminiModel.getVisualizationType(
        isMaxZoomLevel
      );
      const { min: minValue, max: maxValue } = findExtentByTrackType(
        tabularData,
        isStackedBarChart
      );
