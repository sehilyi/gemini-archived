...
export function drawBarChart(
  ...
  const colorCategories = encodedFields["color"]
    ? Array.from(new Set(data.map((d) => d[encodedFields["color"]])))
    : [];
  const rowCategories = encodedFields["row"]
    ? Array.from(new Set(data.map((d) => d[encodedFields["row"]])))
    : [];
...
export function drawStackedBarChart(
  ...
  const sprite = new HGC.libraries.PIXI.Sprite(texture);
  sprite.width =
    trackInfo._xScale(tileX + tileWidth) - trackInfo._xScale(tileX);
