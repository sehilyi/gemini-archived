...
export function drawLineCharts(
  HGC: any,
  trackInfo: any,
  tile: any,
  alt: boolean
  ...
  for (let i = 0; i < matrix[0].length; i++) {
    ...
    graphics.lineStyle(
      1,
      trackInfo.colorHexMap[trackInfo.geminiModel.getColorRange(alt)[i]],
      1
    );

    for (let j = 0; j < matrix.length; j++) {
      // 3070 or something
      const x = trackInfo._xScale(
        tileX + (j * tileWidth) / trackInfo.tilesetInfo.tile_size
      );
      ...
      trackInfo.addSVGInfo(
        ...
        x,
        y,
        trackInfo.geminiModel.getColorRange(alt)[i]
      );
