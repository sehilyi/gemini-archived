...
export function renderCircularLayout(
  ...
  if (gm.layout?.direction === "horizontal") {
    ...
    gm.tracks.forEach((track, i) => {
      if (IsNotEmptyTrack(track)) {
        ...
        const endAngle =
          startAngle + (Math.PI * 2 * length) / totalLength - angleGap;
        trackInfo.push({
          track,
          arc: {
  ...
  g.append("g")
    .attr(
      "transform",
      `translate(${boundingBox.x + totalRadius}, ${
        boundingBox.y + totalRadius
      })`
    )
    .selectAll("path")
    .data(trackInfo)
    ...
    .attr("d", (d) =>
      d3.arc().innerRadius(d.arc.innerRadius).outerRadius(d.arc.outerRadius)(
        d.arc
      )
    );
}
