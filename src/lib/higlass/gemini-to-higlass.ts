...
export function compiler(
  ...
  if (IsHiGlassTrack(track) && IsDataDeep(track.data)) {
    const { server, tilesetUid } = parseServerAndTilesetUidFromUrl(
      track.data.url
    );
    ...
    const xDomain =
      isXGenomic && IsChannelDeep(track.x)
        ? (track.x.domain as Domain)
        : undefined;
    const yDomain =
      isYGenomic && IsChannelDeep(track.y)
        ? (track.y.domain as Domain)
        : undefined;
    const trackDirection =
      isXGenomic && isYGenomic
        ? "both"
        : isXGenomic
        ? "horizontal"
        : "vertical";
    const trackType = IsShallowMark(track.mark)
      ? track.mark
      : IsMarkDeep(track.mark)
      ? track.mark.type
      : "unknown";
